...
type AssignmentEnrollmentsSearchParams = {
  status?: string;
  message?: string;
  termId?: string;
};
...
export default async function AssignmentEnrollmentsPage({
  ...
  searchParams: Promise<AssignmentEnrollmentsSearchParams>;
}) {
  ...
  const selectedTermId =
    params.termId && terms.some((term) => term.id === params.termId) ? params.termId : activeTerm?.id ?? terms[0]?.id ?? "";
  const selectedTerm = terms.find((term) => term.id === selectedTermId) ?? null;
  const filteredEnrollments = selectedTermId ? enrollments.filter((row) => row.termId === selectedTermId) : enrollments;
  ...
  const enrolledCountByClass = new Map(
    classes.map((klass) => [
      klass.id,
      filteredEnrollments.filter((row) => row.class.name.trim().toLowerCase() === klass.name.trim().toLowerCase()).length,
    ]),
  );
    ...
    <>
      ...
      <section className="card card-body d-grid gap-3">
        ...
        <form action={enrollStudentAction} className="grid gap-2 md:grid-cols-4">
          ...
          <label className="d-grid gap-1">
            ...
            <select name="termId" className="form-select" required defaultValue={selectedTermId}>
      ...
      <section className="card card-body">
        <div className="d-flex flex-wrap align-items-center justify-content-between gap-2">
          ...
          {terms.length > 0 ? (
            <label className="d-flex align-items-center text-muted small mb-0">
              Students enrolled in
              <AssignmentsTermSelect terms={terms} selectedTermId={selectedTermId} />
            </label>
          ) : null}
        </div>
        <p className="section-subtle mb-3">
          Enrollment counts below are for {selectedTerm ? `${selectedTerm.sessionLabel} ${selectedTerm.termLabel}` : "the selected term"} only.
        </p>
            ...
            <tbody>
              {classes.map((klass) => (
                <tr key={klass.id}>
                  ...
                  <Td>{enrolledCountByClass.get(klass.id) ?? 0}</Td>
                </tr>
              ))}
              {classes.length === 0 && (
                <tr>
      ...
      <section className="card card-body">
        <h2 className="section-heading">Enrollments</h2>
        <p className="section-subtle mb-3">
          Showing enrollments for {selectedTerm ? `${selectedTerm.sessionLabel} ${selectedTerm.termLabel}` : "the selected term"}.
          You can still filter the list by student, class, or term text below.
        </p>
        <EnrollmentTable rows={filteredEnrollments} removeAction={removeEnrollmentAction} />
      </section>
    </>
  );
}
