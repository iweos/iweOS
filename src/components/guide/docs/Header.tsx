...
export default function Header({ tabs, activeTab, onSelectTab, onOpenSearch, onOpenMobileSidebar }: HeaderProps) {
  return (
    <header className="sticky top-0 z-40 border-b border-slate-200/80 bg-[#fafafa]/90 backdrop-blur dark:border-slate-800 dark:bg-[#0f1117]/90">
      <div className="mx-auto flex h-14 max-w-[1600px] items-center gap-2.5 px-4 sm:px-6">
        <button
          type="button"
        ...
        <nav className="hidden items-center gap-0.5 rounded-xl border border-slate-200 bg-white p-0.5 lg:flex dark:border-slate-800 dark:bg-[#161922]">
          {tabs.map((tab) => (
            <button
              key={tab.id}
              type="button"
        ...
        <button
          ...
          className="mx-auto hidden min-w-[240px] items-center justify-between rounded-xl border border-slate-200 bg-white px-3.5 py-1.5 text-left text-[12px] text-slate-500 shadow-sm transition hover:border-slate-300 lg:flex dark:border-slate-800 dark:bg-[#161922] dark:text-slate-400"
        >
        ...
        <div className="ml-auto flex items-center gap-2">
          ...
          <button
            type="button"
            onClick={onOpenSearch}
            className="hidden items-center gap-2 rounded-lg bg-slate-950 px-3.5 py-1.5 text-[12px] font-semibold text-white transition hover:bg-slate-800 lg:inline-flex dark:bg-[var(--primary)] dark:text-white dark:hover:bg-[var(--primary-strong)]"
          >
          ...
          <Link
            href="/sign-in"
            className="inline-flex items-center rounded-lg border border-slate-200 bg-white px-3.5 py-1.5 text-[12px] font-semibold text-slate-700 transition hover:border-slate-300 hover:text-slate-950 dark:border-slate-700 dark:bg-[#161922] dark:text-slate-200"
          >
            Sign in
          </Link>
        </div>
