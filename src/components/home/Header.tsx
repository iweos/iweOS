...
export default function Header() {
  return (
    <header className="site-header site-hornbill sticky top-0 z-50 border-b border-slate-200/80 bg-[#f8f4ee]/95 backdrop-blur">
          ...
          {navItems.map((item) =>
            item.children?.length ? (
              <div key={item.label} className="group relative py-2">
                ...
                <div className="invisible absolute left-0 top-full z-40 min-w-[180px] pt-2 opacity-0 transition duration-150 group-hover:visible group-hover:opacity-100 group-focus-within:visible group-focus-within:opacity-100">
                  <div className="rounded-xl border border-slate-200 bg-white p-2 shadow-[0_18px_45px_rgba(15,23,42,0.12)]">
